...
class LFWTripletDataset(Dataset):
    def __init__(self, lfw_folder_path, transform):
            ...
            if os.path.isdir(folder_path):
                ...
                for filename in os.listdir(folder_path):
                    ...
                    if len(dets) == 1: # if one face is detected
                       ...
                       count_images_one_person = count_images_one_person + 1
                if count_images_one_person < 2 and len(image_paths)>=1:
                    image_dict.pop(person_name,None)
                    #print('problem detectd in triplet, count < 2 ', folder_path, person_name)

        print('image count for before triplets = ', len(image_dict.keys()))
        triplet_list = []
        ...
        print('Number of trilets =', len(triplet_list))
