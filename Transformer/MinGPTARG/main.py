...
def main():
    ...
    train_loader, val_loader, val_dataset = utils.get_loaders_enwiki8(SEQ_LENGTH,
    BATCH_SIZE)
    ...
    if RESUME_TRAINING == False:
        ...
        checkpoint_data = torch.load('D:\mgovind\MinGPTARG\MinGPT\checkpoint\mingpt_model.pt')
        model.load_state_dict(checkpoint_data['state_dict'])
        optim.load_state_dict(checkpoint_data['optimizer'])
        start = checkpoint_data['epoch']
    for i in tqdm.tqdm(range(start,NUM_BATCHES), mininterval = 10., desc =
    'training'):
        model.train()
        total_loss = 0
        ...
        if i % VALIDATE_EVERY == 0:
            ...
            with torch.no_grad():
                for v in range(0,val_count):
                    ...
                    loss_m = loss.mean()
                    total_loss2 += SEQ_LENGTH * loss_m.item() #loss.float().item()
                    #seq_len
        ...
        if i % GENERATE_EVERY == 0:
            ...
            ckpt_path = os.path.join("D:\mgovind\MinGPTARG\MinGPT\checkpoint\mingpt_model.pt")
