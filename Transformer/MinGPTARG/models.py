...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        B, T, C = x.size() # batch size=B, sequence length=128, embedding dimensionality (n_embd)=192

        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        q, k ,v  = self.c_attn(x).split(self.n_embd, dim=2) # x=(64,128,192)
...
class GPT(nn.Module):
    ...
    def __init__(self,dim, num_unique_tokens=256, num_layers=6, heads=8, dim_head=None, max_seq_len=1024): # modified to match our way of creating model
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            drop = nn.Dropout(0.1),
            h = nn.ModuleList([Block(dim, max_seq_len, heads) for _ in
            range(heads)]), # added Block() modified to Block(dim, max_seq_len, heads)
